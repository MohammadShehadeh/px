...
At each level ask "does the bug still reproduce below this point?" and descend. Flat trees (`${CLAUDE_PLUGIN_ROOT}/rules/components.md`) keep this walk short — if localizing takes more than a few jumps, that is itself a finding.
...
- **Inbound — who references it**: grep the file's import path. Direct imports with no barrels (`${CLAUDE_PLUGIN_ROOT}/rules/structure.md`) mean every consumer is one grep away: `rg "components/date-range-picker"` → the complete consumer list.
...
- The greppable log prefix (`'Error in <fn>::'`, `${CLAUDE_PLUGIN_ROOT}/rules/errors.md`) tells you which layer produced the bad value.
...
- Add the regression test where the decision lives (pure logic, `${CLAUDE_PLUGIN_ROOT}/rules/testing.md`), not where the symptom rendered.
