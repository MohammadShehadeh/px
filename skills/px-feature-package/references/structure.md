...
- **When a feature package is warranted:** the feature is shared by 2+ apps, or genuinely versionable/deployable on its own. **When not:** single-app features — a feature-colocated route folder is the default (`px-conventions` skill: `nextjs` rule); don't manufacture a package for one consumer.
...
- Feature-colocated route folders own their `components/`, `lib/`, `store/`, `hooks/`; only shared code is hoisted (`px-conventions` skill: `nextjs` rule).
