<!-- Copy of skills/px-conventions/references/forms.md so this skill installs standalone — keep in sync. -->

# Forms

Form *logic* is react-hook-form + zod; form *markup* composes the shadcn field primitives (base-layer rules in `px-conventions` skill: `ui-composition` rule). Both halves live here.
...
- Field and submit errors render from a typed `errorKey` resolved at render time — never hardcoded strings (`px-conventions` skill: `errors` rule).
